...
Handle<ScopeInfo> ScopeInfo::Create(Isolate* isolate, Zone* zone, Scope* scope,
  ...
  const bool should_save_class_variable =
      scope->is_class_scope() ? scope->AsClassScope()->ShouldSaveClassVariable()
                              : false;
  ...
  int index = kVariablePartIndex;
  {
    ...
    int flags =
        ...
        HasSavedClassVariableField::encode(should_save_class_variable) |
...
Handle<ScopeInfo> ScopeInfo::CreateForWithScope(
  ...
  int flags =
      ...
      HasSavedClassVariableField::encode(false) |
...
bool ScopeInfo::HasSavedClassVariable() const {
  return HasSavedClassVariableField::decode(Flags());
}
...
int ScopeInfo::SavedClassVariableContextLocalIndex() const {
  if (length() > 0 && HasSavedClassVariableField::decode(Flags())) {
...
int ScopeInfo::ReceiverInfoIndex() const {
  return SavedClassVariableInfoIndex() + (HasSavedClassVariable() ? 1 : 0);
}
