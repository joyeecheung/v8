...
class AccessorTable
    : public base::TemplateHashMap<Literal, Accessors, bool (*)(void*, void*),
                                   ZoneAllocationPolicy> {
 public:
  explicit AccessorTable(Zone* zone)
      : base::TemplateHashMap<Literal, Accessors, bool (*)(void*, void*),
  ...
  Iterator lookup(Literal* literal) {
    ...
    if (it->second == nullptr) {
      it->second = new (zone_) Accessors();
      ordered_accessors_.push_back({literal, it->second});
    }
    return it;
  }

  const std::vector<std::pair<Literal*, Accessors*>>& ordered_accessors() {
    return ordered_accessors_;
  }

 private:
  std::vector<std::pair<Literal*, Accessors*>> ordered_accessors_;
        ...
        switch (property->kind()) {
          ...
          case ClassLiteral::Property::SETTER: {
            ClassAccessors* accessors =
                private_accessors.Add(zone(), property);
            ...
            RegisterAllocationScope register_scope(this);
            RegisterList accessors_reg =
                register_allocator()->NewRegisterList(2);
            if (accessors->getter == nullptr) {
              builder()->LoadNull().StoreAccumulatorInRegister(
                  accessors_reg[0]);
            } else {
              VisitForRegisterValue(accessors->getter->value(),
                                    accessors_reg[0]);
            }
            ...
            if (accessors->setter == nullptr) {
              ...
              VisitForRegisterValue(accessors->setter->value(),
                                    accessors_reg[1]);
            }
            builder()->CallRuntime(Runtime::kCreatePrivateAccessors,
                                   accessors_reg);
            BuildVariableAssignment(property->private_name_var(), Token::INIT,
                                    HoleCheckMode::kElided);
            break;
