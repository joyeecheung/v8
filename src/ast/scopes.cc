...
ClassScope::PrivateName::PrivateName(Variable* variable,
                                     ClassScope::PrivateNameType type) {
  used_ = static_cast<int>(type);
  switch (type) {
...
bool ClassScope::PrivateName::IsAccessor() const {
  if ((used_ & static_cast<int>(ClassScope::PrivateNameType::kGetter)) ||
      (used_ & static_cast<int>(ClassScope::PrivateNameType::kSetter))) {
    return true;
  }
  return false;
}

bool ClassScope::PrivateName::AcceptsAccessor(
    ClassScope::PrivateNameType type) const {
  ...
  if (used_ == static_cast<int>(ClassScope::PrivateNameType::kSetter)) {
    return type == ClassScope::PrivateNameType::kGetter;
  }
  return false;
}

void ClassScope::PrivateName::AddAccessor(Variable* variable,
                                          ClassScope::PrivateNameType type) {
  if (used_ == static_cast<int>(ClassScope::PrivateNameType::kGetter)) {
    ...
    secondary_ = variable;
    used_ |= static_cast<int>(type);
  } else if (used_ == static_cast<int>(ClassScope::PrivateNameType::kSetter)) {
...
ClassScope::PrivateName* ClassScope::PrivateNameMap::Declare(
    Zone* zone, ClassScope* scope, ClassScope::PrivateNameType type,
  ...
  *was_added = p->value == nullptr;
  if (*was_added) {
    DCHECK_EQ(name, p->key);
    Variable* variable = new (zone)
        Variable(scope, name, VariableMode::kConst, NORMAL_VARIABLE,
                 Variable::DefaultInitializationFlag(VariableMode::kConst),
                 MaybeAssignedFlag::kMaybeAssigned);
    ClassScope::PrivateName* private_name =
        new (zone) ClassScope::PrivateName(variable, type);
    p->value = private_name;
    return private_name;
  }

  ClassScope::PrivateName* private_name =
      reinterpret_cast<ClassScope::PrivateName*>(p->value);
  bool isAccessor = (type == ClassScope::PrivateNameType::kGetter ||
                     type == ClassScope::PrivateNameType::kSetter);
  if (isAccessor && private_name->IsAccessor() &&
      private_name->AcceptsAccessor(type)) {
    Variable* variable = new (zone)
        Variable(scope, name, VariableMode::kConst, NORMAL_VARIABLE,
                 Variable::DefaultInitializationFlag(VariableMode::kConst),
                 MaybeAssignedFlag::kMaybeAssigned);
...
void ClassScope::PrivateNameMap::Add(Zone* zone,
                                     ClassScope::PrivateName* private_name) {
  const AstRawString* name = private_name->name();
  Entry* p =
      ZoneHashMap::LookupOrInsert(const_cast<AstRawString*>(name), name->Hash(),
...
ClassScope::PrivateName* ClassScope::PrivateNameMap::Lookup(
    const AstRawString* name) {
  Entry* p = ZoneHashMap::Lookup(const_cast<AstRawString*>(name), name->Hash());
  if (p != nullptr) {
    DCHECK(reinterpret_cast<const AstRawString*>(p->key) == name);
