...
_FUNCTION_KEYWORD_BACKSEARCH_WINDOW = 200
...
class DebuggerBridge:
  ...
  def _resolve_property_string(self, props, prop_name, read_memory):
    ...
    try:
      ...
      # Try to read the full string via the raw character data property.
      # The brief field truncates strings at 80 characters, so reading from
      # the chars/raw_characters property is necessary for longer strings.
      for i in range(result.num_properties):
        ...
        if name in ("chars", "raw_characters") and p.num_values > 0:
          ...
          try:
            ...
            break
      # Fall back to extract the string from the truncated brief.
      brief = ""
      if result.brief:
  ...
  def _position_from_offset(self, script_source, char_offset):
    ...
    # char_offset points to the '(' of the parameter list (V8's scope start
    # position). Search backward for the 'function' keyword to report a more
    # intuitive position. Keep this bounded so malformed or minified source
    # does not turn position decoding into a large backward scan.
    adjusted = char_offset
    search_start = max(0, char_offset - _FUNCTION_KEYWORD_BACKSEARCH_WINDOW)
    func_kw = script_source.rfind("function", search_start, char_offset)
    if func_kw >= 0:
      adjusted = func_kw
    line = script_source.count("\n", 0, adjusted) + 1
    last_newline = script_source.rfind("\n", 0, adjusted)
    column = adjusted + 1 if last_newline == -1 else adjusted - last_newline
    return (line, column)
  ...
  def describe_js_frame(self, frame_pointer, read_memory):
    ...
    try:
      ...
      script_name = self._resolve_property_string(props, "script_name",
                                                  read_memory)
      script_source = self._resolve_property_string(props, "script_source",
                                                    read_memory)
