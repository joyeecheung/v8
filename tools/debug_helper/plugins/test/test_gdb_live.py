...
from .helpers.runtime import get_gdb_test_config
from .helpers.runtime import run_debugger_command


def run_gdb(config, binary_path, run_arguments):
...
class GdbBacktraceTest(unittest.TestCase):
  ...
  @classmethod
  def setUpClass(cls):
    cls.config = get_gdb_test_config()
  ...
  def test_corruption_cases(self):
    ...
    for case in get_corruption_cases(test_dir):
      with self.subTest(corruption=case["name"]):
        output = run_gdb(self.config, self.config.corruption_binary,
                         f'"{case["script"]}"')
