...
class _GdbHintsResolver:
  ...
  def field_offset(self, type_name, field_name):
    ...
    Only valid for byte-aligned fields (pointers, struct members).
    Bit-fields whose `bitpos` is not a multiple of 8 will return a
    wrong offset.
    """
    try:
...
class V8Command(gdb.Command):
  ...
  Currently implements `v8 inspect <addr>`.
  """
...
if os.environ.get("V8_DEBUG_HELPER_LIB_PATH"):
  V8DbgFrameFilter()
  V8Command()
else:
  gdb.write(
      ...
      "libv8_debug_helper shared library and re-source this plugin.\n",
      gdb.STDERR)
