...
helpers used by the GDB and LLDB adapters.
"""
...
_LIBRARY = None
# Prevent the ctypes callback from being garbage-collected while the
# FFI call is in flight by storing a reference at module scope.
...
def _library():
  global _LIBRARY
  if _LIBRARY is None:
    lib_path = os.environ.get("V8_DEBUG_HELPER_LIB_PATH")
    if not lib_path:
      raise RuntimeError(
          "Set V8_DEBUG_HELPER_LIB_PATH to the v8_debug_helper shared library")
    library = ctypes.CDLL(os.path.abspath(lib_path))
